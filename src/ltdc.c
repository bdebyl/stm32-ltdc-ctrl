...
static pin_definitions ltdc_pins[5] = {
    {RCC_GPIOA, GPIOA, LCD_R4 | LCD_R5 | LCD_G2 | LCD_B5},
    {RCC_GPIOB, GPIOB, LCD_R3 | LCD_R6 | LCD_G4 | LCD_G5 | LCD_B7},
    {RCC_GPIOC, GPIOC, LCD_R2 | LCD_G6},
    {RCC_GPIOD, GPIOD, LCD_G7 | LCD_B2 | LCD_IM0 | LCD_IM1 | LCD_IM2 | LCD_IM3},
    {RCC_GPIOG, GPIOG, LCD_R7 | LCD_G3 | LCD_B3 | LCD_B4}};
...
void init_ltdc_gpio() {
    ...
    uint32_t i;
    for (i = 0; i < 5; ++i) {
        rcc_periph_clock_enable(ltdc_pins[i].rcc);

        gpio_mode_setup(ltdc_pins[i].gpio, GPIO_MODE_AF, GPIO_PUPD_NONE,
                        ltdc_pins[i].gpio);

        gpio_set_output_options(ltdc_pins[i].gpio, GPIO_OTYPE_PP,
                                GPIO_OSPEED_50MHZ, ltdc_pins[i].gpio);

        gpio_set_af(ltdc_pins[i].gpio, GPIO_AF14, ltdc_pins[i].gpio);
    }
    ...
    // gpio_mode_setup(GPIOG, GPIO_MODE_AF, GPIO_PUPD_NONE, GPIO7);
    // gpio_set_output_options(GPIOG, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, GPIO7);
